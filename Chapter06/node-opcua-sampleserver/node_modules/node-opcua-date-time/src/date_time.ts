...
const offsetFactor1601 = (function offset_factor_1601() {
...
exports.offset_factor_1601 = offsetFactor1601;
 ...
 * @returns {[high,low]}
 */
function bn_dateToHundredNanoSecondFrom1601_fast(date: Date, picoseconds: number) {
...
exports.bn_dateToHundredNanoSecondFrom1601Excess = function bn_dateToHundredNanoSecondFrom1601Excess(date: Date, picoseconds: number) {
    // 100 nano seconds = 100 x 1000 picoseconds
    return (picoseconds !== undefined) ? picoseconds % 100000 : 0;

};

function bn_hundredNanoSecondFrom1601ToDate_fast(high: number, low: number, picoseconds: number) {
    ...
    (date as any).picoseconds = excess100nanoInPico + ((picoseconds !== null) ? picoseconds : 0);
    return date;
}
...
interface PreciseClockEx extends PreciseClock {
    tick: number[];
}
 ...
 * @return PreciseClock
 */
function getCurrentClockWithJavascriptDate(): PreciseClock {
    const now = new Date();
    if (lastNowDate && now.getTime() === lastNowDate.getTime()) {
        lastPicoseconds += smallTickPicosecond; // add "100 nano" second which is hte resolution of OPCUA DateTime
    } else {
        lastPicoseconds = 0;
        lastNowDate = now;
    }
...
function getCurrentClockWithProcessHRTime(): PreciseClock {
